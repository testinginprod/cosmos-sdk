...
func TestTrackHistoricalInfo(t *testing.T) {
	...
	app.StakingKeeper.HistoricalInfo.Insert(ctx, 4, hi4)
	app.StakingKeeper.HistoricalInfo.Insert(ctx, 5, hi5)
	recv, err := app.StakingKeeper.HistoricalInfo.Get(ctx, 4)
	require.NoError(t, err)
	require.Equal(t, hi4, recv)
	recv, err = app.StakingKeeper.HistoricalInfo.Get(ctx, 5)
	require.NoError(t, err)
	...
	recv, err = app.StakingKeeper.HistoricalInfo.Get(ctx, 10)
	require.NoError(t, err, "GetHistoricalInfo failed after BeginBlock")
	require.Equal(t, expected, recv, "GetHistoricalInfo returned unexpected result")

	// Check HistoricalInfo at height 5, 4 is pruned
	recv, err = app.StakingKeeper.HistoricalInfo.Get(ctx, 4)
	require.Error(t, err, "GetHistoricalInfo did not prune earlier height")
	require.Equal(t, types.HistoricalInfo{}, recv, "GetHistoricalInfo at height 4 is not empty after prune")
	recv, err = app.StakingKeeper.HistoricalInfo.Get(ctx, 5)
	require.Error(t, err, "GetHistoricalInfo did not prune first prune height")
