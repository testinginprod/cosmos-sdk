...
func (i MultiIndex[IK, PK, V]) First(ctx sdk.Context, ik IK) (PK, error) {
	it := i.ExactMatch(ctx, ik)
	if !it.Valid() {
		var pk PK
		return pk, fmt.Errorf("%w: no index matches for reference key", ErrNotFound)
	}
	return it.PrimaryKey(), nil
}
